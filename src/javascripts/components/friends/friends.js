...
const showFriends = (friends) => {
  ...
  friends.forEach((friend) => {
    ...
    domString += `<input type="radio" id="radio1_${friend.id}" name="radios-button_${friend.id}" class="custom-control-input">`;
    domString += `<label class="custom-control-label" for="radio1_${friend.id}">YES</label>`;
    domString += '</div>';
    domString += '<div class="custom-control custom-radio custom-control-inline">';
    domString += `<input type="radio" id="radio2_${friend.id}" name="radios-button_${friend.id}" class="custom-control-input">`;
    domString += `<label class="custom-control-label" for="radio2_${friend.id}">No</label>`;
    domString += '</div>';
    domString += '<div class="custom-control custom-radio custom-control-inline">';
    domString += `<input type="radio" id="radio3_${friend.id}" name="radios-button_${friend.id}" class="custom-control-input">`;
